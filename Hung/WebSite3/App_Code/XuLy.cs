...
namespace lanhnt
{
    ...
    public class ChucNang
    {
        ...
        public string LoadMenu(int CNMe, int level)
        {
            ...
            Lenh.CommandType = CommandType.StoredProcedure;
            SqlParameter ThamSo = new SqlParameter();
            ThamSo = Lenh.Parameters.AddWithValue("@CNMe", CNMe);
            SqlDataReader DocDL;
            BaoVe.Open();
            DocDL = Lenh.ExecuteReader();
            if (!DocDL.HasRows)
            ...
            else
            {
                ...
                while(DocDL.Read())
                {
                    KetQua += "<li><a href='<%# Eval('Link')%>'><span>" + DocDL["TenCN"] + "</span></a></li>";
        ...
        public string menu(int menuid)
        {
            ...
            Lenh.CommandType = CommandType.StoredProcedure;
            SqlParameter ThamSo = new SqlParameter();
            ThamSo = Lenh.Parameters.AddWithValue("@CNMe", menuid);
            SqlDataReader DocDL;
            BaoVe.Open();
            DocDL = Lenh.ExecuteReader();
            //đọc tất cả category con từ cơ sở dữ liệu có id mẹ là menuid (cần thiết phải có cột id category mẹ)
            //sau đó:
            if (DocDL.Read()==true)//kiểm tra số phần tử con lấy được
            {
                result += "<li><a href='#'>" + DocDL["TenCN"].ToString() +"</a></li>";
                result = result + "</ul>";
                menu(Convert.ToInt32(DocDL["Ma"]));
                return result;
            }
            else //nếu ko có phần tử nào
            {
                return result;
            }
        }         
        ...
        {
            SqlConnection BaoVe = new SqlConnection("server=(local)\\SQLEXPRESS;uid=sa;pwd=123;database=QuanLyCongVan");
            SqlDataAdapter XeTai = new SqlDataAdapter("Menu_DS", BaoVe);
            DataTable ThungChua = new DataTable();
            BaoVe.Open();
            XeTai.Fill(ThungChua);
            BaoVe.Close();
